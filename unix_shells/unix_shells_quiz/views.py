...
# renders the About page located at unix_shells_quiz/unix_shells_quiz/index.html
...
def logout(request):
    return render(request, 'unix_shells_quiz/logout.html')
...
def quiz_selection(request):
    if request.method == 'POST':
        ...
        f = open('unix_shells_quiz/user_response.py', 'w')
        ...
        f.close()
    return render(request, 'unix_shells_quiz/quiz_selection.html')
...
#        f.close()
        ...
        f = open('unix_shells_quiz/user_response.py', 'rb')
        ...
        user_response = smart_text(user_response)
    if user_response == '':
        return render(request, 'unix_shells_quiz/index.html')
    if user_response == '1':
        latest_question_list = Question.objects.filter(category="Shell History")
    if user_response == '2':
    ...
    return render(request, 'unix_shells_quiz/index.html', context)


def git_resources(request):
        return render(request, 'unix_shells_quiz/resources.html')

def detail(request, question_id):
    question = get_object_or_404(Question, pk=question_id)
    return render(request, 'unix_shells_quiz/detail.html', {'question': question})
...
def results(request, question_id):
    ...
    return render(request, 'unix_shells_quiz/results.html', context)
...
def reset_scores(request):
    ...
    for answer_item in answers:
        ...
        answer_item.save()
    return render(request, 'unix_shells_quiz/reset_scores.html')

def short_answer_quiz_categories(request):
    if request.method == 'POST':
        return render(request, 'unix_shells_quiz/index.html')
    else:
        return render(request, 'unix_shells_quiz/short_answer_quiz_categories.html')

# The following function renders the Tools Page

def tools(request):
    return render(request, 'unix_shells_quiz/tools.html')
...
def feynman_technique_quiz(request):
    ...
    return render(request, 'unix_shells_quiz/feynman_technique_quiz.html', context)

def ftq_detail(request, question_id):
    question = get_object_or_404(Ftq_Question, pk=question_id)
    return render(request, 'unix_shells_quiz/ftq_detail.html', {'question': question})
...
def ftq_results(request, question_id):
    ...
    return render(request, 'unix_shells_quiz/ftq_results.html', context)
...
def ftq_reset_scores(request):
    ...
    """    for answer_item in answers:
        ...
        answer_item.save()
    """
    return render(request, 'unix_shells_quiz/ftq_reset_scores.html')
...
def multiple_choice_quiz(request):
    ...
    return render(request, 'unix_shells_quiz/multiple_choice_quiz.html', context)


def multiple_choice_quiz_detail(request, question_id):
    question = get_object_or_404(Mc_Question, pk=question_id)
    return render(request, 'unix_shells_quiz/multiple_choice_quiz_detail.html', {'question': question})
...
def vote(request, question_id):
    ...
    try:
        ...
        f = open('unix_shells_quiz/selected_answer.py', 'w')
        # the line below write the text 'user_response = ' and concats the user_response the str function gets rid of u in front of string
        f.write(str(user_selected_answer))
        f.close()
    except (KeyError, Answer.DoesNotExist):
        # Redisplay the question voting form.
        return render(request, 'unix_shells_quiz/multiple_choice_quiz_detail.html', {
            'question': question,
        ...
        # Note: unix_shells_quiz:multiple_choice_quiz_results calls the multiple_choice_quiz_results function
        context = {'selected_answer': selected_answer}
        return HttpResponseRedirect(reverse('unix_shells_quiz:multiple_choice_quiz_results', args=(question.id,)))
        #return redirect('unix_shells_quiz:multiple_choice_quiz_results', kwargs=(context, question_id))


def multiple_choice_quiz_results(request, question_id):
    question = get_object_or_404(Question, pk=question_id)
    user_selected_answer = ""
    f = open('unix_shells_quiz/selected_answer.py', 'rb')
    ...
    return render(request, 'unix_shells_quiz/multiple_choice_quiz_results.html', context )
